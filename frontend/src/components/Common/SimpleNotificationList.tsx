...
const SimpleNotificationList: React.FC<SimpleNotificationListProps> = ({ notifications, onRemove }) => {
  ...
  const getBackgroundColor = (type: SimpleNotification['type']) => {
    switch (type) {
      case 'success':
  ...
  const getProgressBarColor = (type: SimpleNotification['type']) => {
    switch (type) {
      case 'success':
        return 'bg-green-500';
      case 'error':
